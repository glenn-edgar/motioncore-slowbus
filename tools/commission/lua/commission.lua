...
do
    ...
    while arg[i] do
        ...
        elseif a == "--add-files" then opt.add_files = true   -- push only extra_files, no format
...
local push_hil = not opt.add_files
local files = push_hil and compile_files(node.attrs) or {}
local expect_mode = sd.MODES[node.attrs.sub]

-- extra files (e.g. a boot_store's sam_slaves roster) -- generated bytes stored
-- hex in the node attrs; pushed alongside the HIL files (or, with --add-files,
-- instead of them, with no reformat).
local function fromhex(h)
    local t = {}; for i = 1, #h, 2 do t[#t + 1] = string.char(tonumber(h:sub(i, i + 1), 16)) end
    return table.concat(t)
end
local extra, extra_names = {}, {}
if node.attrs.extra_files then
    for name, h in pairs(node.attrs.extra_files) do extra[name] = fromhex(h); extra_names[#extra_names + 1] = name end
    table.sort(extra_names)
end
local push = {}                                    -- ordered {name, bytes} list
if push_hil then for _, k in ipairs(ordered(files)) do push[#push + 1] = { k, files[k] } end end
for _, k in ipairs(extra_names) do push[#push + 1] = { k, extra[k] } end
if #push == 0 then die("nothing to push for %q (no HIL files, no extra_files)", path) end
local function push_names() local n = {}; for _, f in ipairs(push) do n[#n + 1] = f[1] end; return table.concat(n, ",") end
io.write(string.format("target %s : type=%s %s i2c=0x%02X%s  %s=[%s]\n",
    path, tostring(node.attrs.type), node.attrs.sub, node.attrs.i2c or 0,
    node.attrs.boot_store and " BOOT_STORE" or "", opt.add_files and "add" or "push", push_names()))
...
if opt.add_files then
    -- incremental add: the chip must already be THIS node's bound unit (we're only
    -- appending files, never reformatting or rebinding).
    if x_node ~= path then dg:close(); die("--add-files: chip %s is not bound to %q (bound=%s)", uid, path, tostring(x_node or "-")) end
elseif committed and not opt.yes then
    -- guard: already-commissioned chip -> warn + confirm (full re-commission reformats)
...
-- push (HIL files unless --add-files, then extra files)
io.write(opt.add_files and "adding files (no format)...\n" or "pushing...\n")
dg:offline()
if push_hil and not opt.no_erase then dg:file_format() end
for _, f in ipairs(push) do dg:file_put(f[1], f[2]) end

-- read back + byte-compare (still offline, same connection)
io.write("verifying (read-back)...\n")
local verify_ok = true
for _, f in ipairs(push) do
    local got = dg:file_get(f[1])
    local ok = (got == f[2])
    io.write(string.format("  %-5s %s\n", f[1], ok and "OK" or ("MISMATCH got=" .. hex(got) .. " want=" .. hex(f[2]))))
    verify_ok = verify_ok and ok
end
dg:close()                       -- reboot -> apply config
if not verify_ok then die("read-back mismatch -- NOT binding") end

-- reopen by UID and verify the mode applied -- only when we wrote HIL (idnt);
-- --add-files leaves the mode untouched.
local mode_ok, applied = true, cur_mode
if push_hil then
...
-- KB update: bind/rebind only on a full commission (--add-files keeps the binding)
if not opt.add_files then
    if repurpose then
        if x_node and x_node ~= path then
...
-- status carries the denormalized PURPOSE (type/mode/i2c/boot_store) so the
-- container's USB controller can classify the chip from its UID -> node.
ns:set_status(path, { state = mode_ok and "commissioned" or "mode-mismatch",
                      type = node.attrs.type, mode = applied, i2c = node.attrs.i2c,
                      boot_store = node.attrs.boot_store or false,
                      files = push_names(), verified = verify_ok,
